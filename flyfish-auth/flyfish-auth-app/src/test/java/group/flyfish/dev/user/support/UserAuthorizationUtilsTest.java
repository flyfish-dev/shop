...
class UserAuthorizationUtilsTest {
    ...
    @Test
    void readsGithubProfileFromSanitizedAuthorizationFields() {
        PortalUserOauth oauth = new PortalUserOauth();
        oauth.setType(OAuthType.GITHUB);
        oauth.setOpenid("123456");
        oauth.setUserInfo("{\"id\":\"123456\",\"login\":\"octocat\",\"access_token\":\"secret\"}");
        oauth.setLoginName("octocat");
        oauth.setDisplayName("The Octocat");
        oauth.setEmail("octocat@example.com");

        PortalUserVo user = new PortalUserVo();
        user.setId(100L);
        user.setAuthorizations(Map.of("github", oauthVo(oauth).withoutUserInfo()));

        Map<String, Object> profile = UserAuthorizationUtils.getGithubProfile(user);
        assertEquals("123456", profile.get("id"));
        assertEquals("octocat", profile.get("login"));
        assertEquals("The Octocat", profile.get("display_name"));
        assertEquals("octocat@example.com", profile.get("email"));
        assertFalse(profile.containsKey("access_token"));
    }
