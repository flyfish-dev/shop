...
class SupportTicketNotificationServiceImplTest {
    ...
    private static class Fixture {
        ...
        private final PortalUserOauthRepository portalUserOauthRepository = mock(PortalUserOauthRepository.class);
        ...
        private final SupportTicketNotificationServiceImpl service = new SupportTicketNotificationServiceImpl(
                portalUserRepository, portalUserOauthRepository, properties, messageFactory, mailNotifier, wechatNotifier);
        ...
        private Fixture() {
            ...
            when(portalUserOauthRepository.findAllByTypeAndOpenid(OAuthType.GITEA, "1")).thenReturn(Flux.empty());
        }
        ...
        private void seedCustomer(boolean hasEmail, boolean hasWechat) {
            ...
            when(portalUserOauthRepository.findAllByUserIdAndType(100L, OAuthType.WECHAT))
                    .thenReturn(hasWechat ? Flux.just(oauth(100L, OAuthType.WECHAT, "wechat-customer-openid")) : Flux.empty());
        }

        private void seedMaintainer(boolean hasEmail, boolean hasWechat) {
            when(portalUserOauthRepository.findAllByTypeAndOpenid(OAuthType.GITEA, "1"))
                    .thenReturn(Flux.just(oauth(1L, OAuthType.GITEA, "1")));
            ...
            when(portalUserRepository.findById(1L)).thenReturn(Mono.just(admin));
            when(portalUserOauthRepository.findAllByUserIdAndType(1L, OAuthType.WECHAT))
                    .thenReturn(hasWechat ? Flux.just(oauth(1L, OAuthType.WECHAT, "wechat-admin-openid")) : Flux.empty());
        }
        ...
        private PortalUserOauth oauth(Long userId, OAuthType type, String openid) {
