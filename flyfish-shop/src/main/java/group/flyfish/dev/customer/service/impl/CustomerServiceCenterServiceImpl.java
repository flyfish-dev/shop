...
public class CustomerServiceCenterServiceImpl implements CustomerServiceCenterService, CustomerMessageGateway {
    ...
    private Mono<CustomerConversation> findOrCreateConversation(String openid, Optional<PortalUserOauth> oauth) {
        return conversationRepository.findByWechatOpenid(openid)
                .flatMap(existing -> {
                    applyIdentity(existing, oauth);
    ...
    private Mono<CustomerConversation> findOrCreateConversation(Long targetUserId) {
        return conversationRepository.findAllByUserId(targetUserId)
                .next()
                .switchIfEmpty(oauthRepository.findAllByUserIdAndType(targetUserId, OAuthType.WECHAT)
                        .next()
                        .flatMap(oauth -> findOrCreateConversation(oauth.getOpenid(), Optional.of(oauth))))
                .switchIfEmpty(userRepository.findById(targetUserId)
                        .switchIfEmpty(Mono.error(new BusinessException("USER_NOT_FOUND", "客户不存在")))
                        .flatMap(this::createWebConversation));
    }

    private Mono<CustomerConversation> createConversation(String openid, Optional<PortalUserOauth> oauth) {
    ...
    private Mono<CustomerConversation> createWebConversation(PortalUser user) {
        CustomerConversation conversation = baseWebConversation(user.getId());
        conversation.setDisplayName(StringUtils.defaultIfBlank(user.getUsername(), "用户 " + user.getId()));
    ...
    private void applyIdentity(CustomerConversation conversation, Optional<PortalUserOauth> optional) {
        if (optional.isEmpty()) {
        ...
        PortalUserOauth oauth = optional.get();
        PortalUserOauthVo oauthVo = new PortalUserOauthVo(oauth);
        conversation.setUserId(oauth.getUserId());
    ...
    private Mono<PortalUserOauth> currentWechatAuthorization(PortalUserVo user) {
        return oauthRepository.findAllByUserIdAndType(user.getId(), OAuthType.WECHAT).next();
    }
    ...
    private Mono<CustomerConversation> toViewConversation(CustomerConversation conversation) {
        ...
        return userRepository.findById(conversation.getUserId())
                .map(user -> {
    ...
    private Mono<List<CustomerMessageVo>> toMessageVos(CustomerConversation conversation, boolean managerView,
        ...
        Mono<Map<Long, PortalUser>> adminUsers = adminIds.isEmpty()
                ? Mono.just(Map.of())
                : userRepository.findAllById(adminIds).collectMap(PortalUser::getId);
    ...
    private CustomerMessageVo toMessageVo(CustomerConversation conversation, boolean managerView,
                                          CustomerMessage message, Map<Long, PortalUser> adminUsers) {
        ...
        if (CustomerMessage.SenderRole.ADMIN.name().equals(message.getSenderRole())) {
            PortalUser admin = adminUsers.get(message.getSenderId());
