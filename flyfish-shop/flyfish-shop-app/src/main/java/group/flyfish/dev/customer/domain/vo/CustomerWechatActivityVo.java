...
public class CustomerWechatActivityVo {
    ...
    public CustomerWechatActivityVo(CustomerWechatActivity activity) {
        ...
        this.displayName = resolveDisplayName(activity);
    ...
    private String resolveDisplayName(CustomerWechatActivity activity) {
        if (activity.getUserId() == null && StringUtils.isNotBlank(activity.getWechatOpenid())) {
            return FunNicknameGenerator.generate(activity.getWechatOpenid());
        }
        return StringUtils.defaultIfBlank(activity.getDisplayName(),
                FunNicknameGenerator.generate(activity.getWechatOpenid()));
    }
}
