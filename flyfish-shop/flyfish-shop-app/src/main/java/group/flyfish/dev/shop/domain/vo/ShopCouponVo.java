...
public class ShopCouponVo {
    ...
    private String resolveDisplayRule(ShopCoupon coupon) {
        ...
        if (coupon.getType() == ShopCoupon.Type.DISCOUNT) {
            BigDecimal maxDiscount = coupon.getMaxDiscountAmount() == null
                    ? BigDecimal.ZERO
                    : coupon.getMaxDiscountAmount();
            String cap = maxDiscount.signum() > 0
                    ? "，最高减" + maxDiscount.stripTrailingZeros().toPlainString()
                    : "";
            return prefix + "享" + discount.stripTrailingZeros().toPlainString() + "折" + cap;
        }
