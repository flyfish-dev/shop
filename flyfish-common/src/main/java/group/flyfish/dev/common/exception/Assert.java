...
public class Assert {
     ...
     * <p>Call {@link #isTrue} if you wish to throw an {@code ServiceException}
     ...
     */
    public static void isFilePatterns(MultipartFile file, String[] filePatterns) {
        if (Stream.of(filePatterns).noneMatch(filePattern -> file.getOriginalFilename().endsWith(filePattern))) {
            throw new ServiceException("文件格式不正确，必须是" + join(filePatterns, ",") + "中的一种！");
        }
    }
